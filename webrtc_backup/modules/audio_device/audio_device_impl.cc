...
int32_t AudioDeviceModuleImpl::GetPlayoutDevice(
  char name[kAdmMaxDeviceNameSize],
  char guid[kAdmMaxGuidSize]) {
  RTC_LOG(INFO) << __FUNCTION__;
  CHECKinitialized_();
  ...
  if (audio_device_->GetPlayoutDevice(name, guid) == -1) {
    return -1;
  }
  if (name != NULL) {
    RTC_LOG(INFO) << "output: name = " << name;
...
int32_t AudioDeviceModuleImpl::GetRecordingDevice(
  char name[kAdmMaxDeviceNameSize],
  char guid[kAdmMaxGuidSize]) {
  RTC_LOG(INFO) << __FUNCTION__;
  CHECKinitialized_();
  ...
  if (audio_device_->GetRecordingDevice(name, guid) == -1) {
    return -1;
  }
  if (name != NULL) {
    RTC_LOG(INFO) << "output: name = " << name;
