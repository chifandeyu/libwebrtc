...
int32_t AudioDeviceWindowsCore::GetPlayoutDevice(
  char name[kAdmMaxDeviceNameSize],
  char guid[kAdmMaxGuidSize]) {
  const int16_t nDevices(PlayoutDevices());  // also updates the list of devices
  
  if(nDevices < 0 || (name == NULL)) {
    return -1;
  }
  ...
  // Get the endpoint device's friendly-name
  if(_ptrDeviceOut) {
    ret = _GetDeviceName(_ptrDeviceOut, szDeviceName, bufferLen);
  } else{
    ret = _GetDefaultDeviceName(eRender, eCommunications, szDeviceName,
                              bufferLen);
  }
  ...
  if(_ptrDeviceOut) {
    ret = _GetDeviceID(_ptrDeviceOut, szDeviceName, bufferLen);
  } else {
    ret =
      _GetDefaultDeviceID(eRender, eCommunications, szDeviceName, bufferLen);
  }
...
int32_t AudioDeviceWindowsCore::GetRecordingDevice(
  char name[kAdmMaxDeviceNameSize],
  char guid[kAdmMaxGuidSize]) {
  ...
  // Get the endpoint device's friendly-name
  if(_ptrDeviceIn) {
    ret = _GetDeviceName(_ptrDeviceIn, szDeviceName, bufferLen);
  } else{
    ret = _GetDefaultDeviceName(eCapture, eCommunications, szDeviceName,
                              bufferLen);
  }
  ...
  // Get the endpoint ID string
  if(_ptrDeviceIn) {
    ret = _GetDeviceID(_ptrDeviceIn, szDeviceName, bufferLen);
  } else {
    ret =
      _GetDefaultDeviceID(eCapture, eCommunications, szDeviceName, bufferLen);
  }
