...
HRESULT CoreAudioBase::OnSimpleVolumeChanged(float new_simple_volume,
  ...
  bool withBraces = true;
	sprintf(buffer, "%s%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X%s", withBraces ? "{" : "",
    event_context->Data1,
    event_context->Data2,
    event_context->Data3,
    event_context->Data4[0],
    event_context->Data4[1],
    event_context->Data4[2],
    event_context->Data4[3],
    event_context->Data4[4],
    event_context->Data4[5],
    event_context->Data4[6],
    event_context->Data4[7],
    withBraces ? "}" : "");
  RTC_DLOG(INFO) << "----OnSimpleVolumeChanged new_simple_volume: " << new_simple_volume
  << " new_mute: " << new_mute << "context: " << buffer;
