...
bool RTCDesktopMediaListImpl::GetThumbnail(scoped_refptr<MediaSource> source, bool notify, bool async) {
  auto func = [this, source, notify] {
  ...
  if (async) {
    thread_->PostTask(ToQueuedTask(task_safety_.flag(), func));
  } else {
    func();
  }
  return true;
...
bool MediaSourceImpl::UpdateThumbnail(bool async) {
  return mediaList_->GetThumbnail(this, false, async);
}
